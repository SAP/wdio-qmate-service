...
export class UI5ControlDataInjector {
  public static injectDataForProperties(
  ...
  private static injectAttributes(
    ...
    keys.forEach((key) => {
      ...
      const sBindingDataStr = UI5ControlDataInjector.getBindingInfoDataString(
        oControl,
        key
      );
  ...
  private static injectBindingContextPaths(
    domElement: HTMLElement,
    oControl: UI5Control
  ): void {
  ...
  private static getBindingInfoDataString(
    oControl: UI5Control,
    key: string
  ): string {
