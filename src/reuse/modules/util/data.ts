...
export class Data {
   ...
   * @param {String} [source="data"] - The source key defined under params.import of the config file.
   * @param {Object} [options] - The options object. Default: { useBase64Input: false, useBase64Output: false, includeRepoUrl: true }
  ...
  getSecureData(filename: string, source: string = "data", options?: DecryptionOptions): object {
    ...
    const mergedOptions = { ...DEFAULT_DECRYPTION_OPTIONS, ...options };
    ...
    if (browser.config.params && browser.config.params.import && browser.config.params.import[source]) {
      if (browser.config.params.import[source][filename]) {
        ...
        if (!this._alreadyDecryptedData.includes(dataIdentifier) && privateKeyFound) {
          this._decryptRecursively(data, mergedOptions);
        }
   ...
   * @param {Object} options - The decryption options. Default: { useBase64Input: false, useBase64Output: false, includeRepoUrl: true }
  ...
  decrypt(data: string | Array<string>, options: DecryptionOptions): string {
    ...
    return global.util.data.decrypt(data, mergedOptions); // Function is defined under: scripts\hooks\utils\decryption.js
  }
