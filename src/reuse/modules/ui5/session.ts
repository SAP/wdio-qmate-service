...
export class Session {
   ...
   */
  async login (username: string, password = "Welcome1!", verify = false, timeout: number = customTimeout || 30000) {
   ...
   */
  async loginFiori(username: string, password = "Welcome1!", verify = false) {
    if (!username) {
      throw new Error("Please provide a valid username.");
    }
   ...
   */
  async loginSapCloud(username: string, password = "Welcome1!", verify = false) {
    if (!username) {
      throw new Error("Please provide a valid username.");
    }
   ...
   */
  async loginCustom(username: string, password = "Welcome1!", usernameFieldSelector: string, passwordFieldSelector: string, logonButtonSelector: string, verify = false) {
    if (!username) {
      throw new Error("Please provide a valid username.");
    }
  ...
  async loginCustomViaConfig(username: string, password = "Welcome1!", verify = false) {
    try {
   ...
   */
  async switchUser(username: string, password = "Welcome1!", authenticator: any, wait = 10000) {
    await this.logout();
  ...
  // =================================== HELPER ===================================
  private async _loginWithUsernameAndPassword(username: string, password = "Welcome1!", authenticator = ui5.authenticators.fioriForm, verify = false, messageSelector?: string) {
