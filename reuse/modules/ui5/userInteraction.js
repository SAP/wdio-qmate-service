...
const UserInteraction = function () {
  ...
  this.click = async function (selector, index = 0, timeout = 30000) {
    ...
    await browser.waitUntil(async function () {
      elem = await ui5.locator.getDisplayedElement(selector, index, timeout);
  ...
  this.clickListItem = async function (selector, index = 0, timeout = 30000) {
    const elem = await ui5.locator.getDisplayedElement(selector, index, timeout);
  ...
  this.selectBox = async function (selector, value, index = 0) {
    ...
    if (value) {
      ...
      await ui5.locator.scrollToElement(ui5ControlProperties);
  ...
  async function clearHelper(selector, index = 0, timeout = 30000) {
    ...
    if (selector) {
      await ui5.userInteraction.click(selector, index, timeout);
      id = await ui5.locator.getElementId(selector, index, timeout);
