...
const Session = function () {
   ...
   */
  this.login = async function (username, password = "Welcome1!", verify = false, timeout = 30000) {
    if (!username) {
    ...
    try {
      await browser.waitUntil(async function () {
        try {
          ...
          const elem = await nonUi5.locator.getElementByCss(fioriFormId, 0, 7500);
          await nonUi5.assertion.isVisible(elem);
        ...
        try {
          ...
          const elem = await nonUi5.locator.getElementByCss(sapCloudFormId, 0, 7500);
          await nonUi5.assertion.isVisible(elem);
   ...
   */
  this.loginCustom = async function (username, password = "Welcome1!", usernameFieldSelector, passwordFieldSelector, logonButtonSelector, verify = false) {
    if (!username) {
      throw new Error("Please provide a valid username.");
    }
  ...
  this.loginCustomViaConfig = async function (username, password = "Welcome1!", verify = false) {
    try {
   ...
   * await ui5.common.session.switchUser("PURCHASER", "Welcome1!", authenticator, 30000);
   */
