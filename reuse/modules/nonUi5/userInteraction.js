...
const UserInteraction = function () {
   ...
   */
  this.click = async function (element, timeout = process.env.QMATE_CUSTOM_TIMEOUT.qmateCustomTimeout | 30000) {
    await Promise.all([
   ...
   */
  this.clickAndRetry = async function (element, timeout = process.env.QMATE_CUSTOM_TIMEOUT.qmateCustomTimeout | 30000, retries = 3, interval = 5000) {
    if (!element) {
   ...
   */
  this.doubleClick = async function (element, timeout = process.env.QMATE_CUSTOM_TIMEOUT.qmateCustomTimeout | 30000) {
    await Promise.all([
   ...
   */
  this.rightClick = async function (element, timeout = process.env.QMATE_CUSTOM_TIMEOUT.qmateCustomTimeout | 30000) {
    await Promise.all([
      expect(element).toBeDisplayed({
        wait: timeout,
        interval: 100,
