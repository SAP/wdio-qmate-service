describe("Test 'getBindingContextPath()' and 'getBindingProperty()' on both element and browser levels", function () {
  ...
  it("should access non-existing/empty binding context properties on both element and browser levels", async function () {
    ...
    await expect(element.getBindingProperty("wrong_property_name")).rejects.toThrow("TypeError: Cannot read properties of undefined (reading 'parts')");
    await expect(browser.getBindingProperty("wrong_property_name", element)).rejects.toThrow("TypeError: Cannot read properties of undefined (reading 'parts')");

    await expect(element.getBindingProperty("")).rejects.toThrow("TypeError: Cannot read properties of undefined (reading 'parts')");
    await expect(browser.getBindingProperty("", element)).rejects.toThrow("TypeError: Cannot read properties of undefined (reading 'parts')");
  });
});