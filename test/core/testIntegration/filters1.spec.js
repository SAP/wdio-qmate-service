...
describe("filters1", function () {
  it("Step 01: click on the first standard item - evaluate browser", async function () {
    ...
    var title = await browser.controlActionInBrowser(
  ...
  it("Step 08: check Page has descendant text - use deep selector structure sibling wrong", async function () {
    ...
    await expect(ui5.element.getDisplayed(ui5ControlProperties, 0, 5000)).rejects.toThrow(/No visible elements found with selector/);
  });
  ...
  it("Step 14: check name is Accessories - check with wrong properties and not mProperties", async function () {
    ...
    await expect(ui5.element.getDisplayed(ui5ControlProperties, 0, 1000)).rejects.toThrow(/No visible elements found with selector/);
  });
  ...
  it("Step 49: check first item with aggregation property", async function () {
    ...
    var ui5ControlProperties = {
      elementProperties: {
        ...
        mProperties: {
          title: "Accessories",
          tooltip: "Open category Accessories"
        }
      },
  ...
  it("Step 52: check first item with association property wrong", async function () {
    ...
    var attribute = "title"; //eg: title, text, value etc.
    var compareValue = "Accessories"; //expected value
    await expect(ui5.assertion.expectAttributeToBe(ui5ControlProperties, attribute, compareValue, Index, 1000)).rejects.toThrow(/No visible elements found with selector/);
  });
  ...
  it("Step 62: check text for specific list item sibling", async function () {
    ...
    var ui5ControlProperties = {
      elementProperties: {
        ...
        mProperties: {
          title: "Audio/Video Cable Kit - 4m",
          number: "29,99",
          numberUnit: "EUR"
        }
      },
  ...
  it("Step 70: enter on Accessories and search wrong element value", async function () {
    ...
    var value = "Flat Basic"; //value to be entered by user
    await expect(ui5.userInteraction.searchFor(ui5ControlProperties, value, 0, 1000, false)).rejects.toThrow(/No visible elements found with selector/);
  });
  ...
  it("Step 75: click  on Object item wrong child property", async function () {
    ...
    await expect(ui5.userInteraction.click(ui5ControlProperties, 0, 1000)).rejects.toThrow(/No visible elements found with selector/);
  });
  ...
  it("Step 85: check if Supplier i18n path is correct wrong binding context", async function () {
    ...
    var Index = 0;
    await expect(ui5.assertion.expectBindingContextPathToBe(ui5ControlProperties, "/Products('HT-1035')", Index, 1000)).rejects.toThrow(/No visible elements found with selector/);
  });
  ...
  it("Step 86: check if Supplier i18n path is correct wrong binding path name", async function () {
    ...
    await expect(ui5.assertion.expectBindingPathToBe(ui5ControlProperties, attribute2, comparePath2, 0, 1000)).rejects.toThrow(/No visible elements found with selector/);
  });
  ...
  it("Step 87: check if Supplier i18n path is correct no model defined", async function () {
    ...
    await expect(ui5.assertion.expectBindingPathToBe(ui5ControlProperties, attribute1, comparePath1, 0, 1000)).rejects.toThrow(/No visible elements found with selector/);
  });
    ...
    var ui5ControlProperties = {
      elementProperties: {
        ...
        mProperties: {
          ...
          text: [{ path: "Weight" }, { path: "WeightUnit" }]
        }
      },
      parentProperties: {
      ...
      nextSiblingProperties: {
        ...
        mProperties: {
          ...
          text: [{ path: "DimensionWidth" }, { path: "Unit" }, { path: "DimensionDepth" }, { path: "DimensionHeight" }]
        }
      }
    };
    var Index = 0;
    ...
    var ui5ControlProperties = {
      ...
      childProperties: {
        ...
        mProperties: {
          bindingContextPath: "/Products('HT-1035')",
          title: { path: "productMeasuresAttributeText" },
          text: [{ path: "DimensionWidths" }, { path: "Unit" }, { path: "DimensionDepth" }, { path: "DimensionHeight" }]
        }
      }
    };
    var Index = 0;
