...
describe("element - getBindingValue for wrong attribute", function () {
  ...
  it("Execution & Verification", async function () {
    ...
    switch (browser.capabilities.browserName) {
      case "Safari":
        await expect(ui5.element.getBindingValue(selector, attribute)).rejects.toThrow("A JavaScript exception occured: undefined is not an object");
        break;
      case "firefox":
        await expect(ui5.element.getBindingValue(selector, attribute)).rejects.toThrow("TypeError: control.getBinding(...) is undefined");
        break;
      default: //chrome and edge
        await expect(ui5.element.getBindingValue(selector, attribute)).rejects.toThrow("javascript error: Cannot read properties of undefined (reading 'getValue')");
        break;
    }
  });
});
...
describe("element - getBindingValue for wrong selector", function () {
  ...
  it("Execution & Verification", async function () {
    ...
    const attribute = "title";
    await expect(ui5.element.getBindingValue(selector, attribute)).rejects.toThrow(/No visible elements found/);
  });
});
