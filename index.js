...
const vyperLoaderSession = require("./scripts/hooks/beforeSession");
const vyperLoader = require("./scripts/hooks/before");
...
module.exports = class CustomWorkerService {
  ...
  async beforeSession(config, capabilities, specs) {
    try {
      await vyperLoaderSession(config, capabilities, specs);
    } catch (e) {
      if (specs && specs[0]) {
                // `specs` variable is an array, but includes only one current spec
        console.error(`vyperLoader() in 'before' hook failed for spec '${specs[0]}'. ${e}`);
      } else {
        console.error(`vyperLoader() in 'before' hook failed. ${e}`);
      }
    }
  }
  ...
  async before(capabilities, specs, browser) {
    ...
    try {
      await vyperLoader(capabilities, specs, browser);
    } catch (e) {
      if (specs && specs[0]) {
                // `specs` variable is an array, but includes only one current spec
        console.error(`vyperLoader() in 'before' hook failed for spec '${specs[0]}'. ${e}`);
